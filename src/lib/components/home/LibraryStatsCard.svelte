<script lang="ts">
	...
	$effect(() => {
		if (booksStatus?.success && booksStatus?.data) {
			...
			setTimeout(
				() => animateNumber(Math.round((data.total_pages || 0) / 1000), (v) => (animatedPages = v)),
				200
			);
			setTimeout(
				() =>
					animateNumber(Math.round((data.total_kwords || 0) / 1000), (v) => (animatedKwords = v)),
				300
			);
			setTimeout(
				() =>
					animateNumber(Math.round((data.total_visits || 0) / 1000), (v) => (animatedVisits = v)),
				400
			);
		}
	});
	...
	<CardContent class="space-y-2 px-4 pb-4">
		...
		{:else if booksStatus?.success && booksStatus?.data}
			<div class="grid grid-cols-2 gap-1 text-center">
				<div>
					<div
						class="text-base font-bold text-blue-600 transition-all duration-300 dark:text-blue-400"
					>
