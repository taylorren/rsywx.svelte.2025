...
export const GET: RequestHandler = async ({ params, url, fetch }) => {
	...
	try {
		...
		const apiUrl = `${RSYWX_API_BASE_URL || 'https://api.rsywx.com/api/v1'}/books/popular/${bookCount}${refresh ? '?refresh=true' : ''}`;
		...
		if (response.ok) {
			...
			console.error('Failed to fetch popular books:', response.status, errorData);
			return json(
				{
					success: false,
